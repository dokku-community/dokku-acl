...
ACLTEST_HOST_DIR ?= $(CURDIR)/tmp/acltest-host

# Optional path or filename relative to /plugin-src/tests passed to bats, e.g.
# `make unit-tests UNIT_TESTS=acl_report.bats`. Defaults to the whole tests
# directory.
UNIT_TESTS ?= .
...
COMPOSE := DOKKU_VERSION=$(DOKKU_VERSION) ACLTEST_HOST_DIR=$(ACLTEST_HOST_DIR) docker compose -f tests/docker-compose.yml
...
clean:
	...
	rm -rf $(ACLTEST_HOST_DIR) 2>/dev/null || sudo rm -rf $(ACLTEST_HOST_DIR)

# --- Native mode: dokku installed on the host. acl needs no supporting services. ---
...
clean-native:
	@echo "acl needs no supporting compose services; nothing to tear down"
