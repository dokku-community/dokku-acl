...
The hooks the plugin implements - `pre-build`, `pre-delete`, `pre-receive-app`, and `user-auth` - are exercised by invoking the plugin's own trigger scripts directly with the dokku plugin environment the CLI would normally export, so no deploy or ssh connection is required. Leading `VAR=VALUE` arguments to the `fire_trigger` helper set the hook's environment, which is how tests simulate an ssh user (`NAME=user1`) or global settings (`DOKKU_SUPER_USER=admin`). `pre-delete` is additionally exercised through a real `apps:destroy`. Global settings that the CLI should see are written to `~dokku/.dokkurc/acl`, the same file the plugin documentation tells users to edit.
